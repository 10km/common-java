 ...
 */
public class Netty3CorsHandler extends SimpleChannelUpstreamHandler implements ChannelDownstreamHandler {

    public static final String ANY_ORIGIN = "*";
    private static String SCHEME_PATTERN = "^https?://";
    ...
    private static boolean isSameOrigin(final String origin, final String host) {
        if (Strings.isNullOrEmpty(host) == false) {
            ...
            final String originDomain = Pattern.compile(SCHEME_PATTERN).matcher(origin).replaceFirst("");
    ...
    private void setAllowMethods(final HttpResponse response) {
    	Iterable<String> names = Iterables.transform(config.allowedRequestMethods(), 
    			new Function<HttpMethod, String>() {
    		@Override
    		public String apply(HttpMethod input) {
    			return input.getName().trim();
    		}
    	});
    	response.headers().set(ACCESS_CONTROL_ALLOW_METHODS,Joiner.on(",").join(names));
    }

    private void setAllowHeaders(final HttpResponse response) {
        response.headers().set(ACCESS_CONTROL_ALLOW_HEADERS, Joiner.on(",").join(config.allowedRequestHeaders()));
    }
	...
	public void handleDownstream(ChannelHandlerContext ctx, ChannelEvent e) throws Exception {
		if (e instanceof MessageEvent) {
			...
			if(messageEvent.getMessage() instanceof HttpResponse 
				...
				setCorsResponseHeaders(request, (HttpResponse)messageEvent.getMessage(), config);
			}
		}
